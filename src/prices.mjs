...
function createApp(database) {
  ...
  app.get("/prices", (req, res) => {
    ...
    const dateNew = parseDate(req.query.date, "new");
    const cost = calculateCost(age, type, dateNew, baseCost, date);
    res.json({ cost });
  });

  function parseDate(dateString, format) {
    if (format === "new" && dateString) return Temporal.PlainDate.from(dateString);
    if (dateString) {
  ...
  function calculateReduction(date, date2) {
    let reduction = 0;
    if (date && date2 && isMonday(date) && !isHoliday(date, date2)) {
      reduction = 35;
    }
    return reduction;
  }
  ...
  function isHoliday(dateNew, dateOld) {
    const date = dateNew;
    const holidays = database.getHolidays();
    for (let row of holidays) {
      let holiday = Temporal.PlainDate.from(row.holiday);
      if (date && holiday && date.year === holiday.year && date.month === holiday.month && dateNew.day === holiday.day) {
        return true;
      }
    }
    return false;
